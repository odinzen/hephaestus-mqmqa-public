"""Olivine Fe-Mg mixing control vs Wood-Kleppa 1981, any MLIP. Usage: olivine_control.py <model>"""
...
def make_calc(name):
    if name=='sevennet':
        from sevenn.calculator import SevenNetCalculator; return SevenNetCalculator(model='7net-0')
    if name.startswith('matgl:'):
        import matgl; from matgl.ext.ase import PESCalculator
        return PESCalculator(matgl.load_model(name.split(':',1)[1]))
...
print(f'{MODEL}: {base.get_chemical_formula()}, {n} M-sites',flush=True)
for k in (0,2,4,6,8):
...
print(f'\n x     {MODEL} H_mix  Wood-Kleppa   diff')
...
print(f'\n{MODEL} RK: L0={L0:.0f} L1={L1:.0f} vs WK 12552,4184; RMS({MODEL}-WK)={rms:.0f} J/mol')
