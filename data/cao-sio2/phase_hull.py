"""Lower-convex-hull phase-diagram sweep that CLASSIFIES invariants.

Replaces the congruent-melting shortcut in diagram.py. At each temperature the stable
state is the lower convex hull of the liquid G(x) curve plus the fixed-composition
solids; sweeping T and reading how the hull changes classifies each compound's melting
as CONGRUENT (melts to a liquid of its own composition) or INCONGRUENT/PERITECTIC
(decomposes to another solid + liquid before it could congruently melt), and locates
eutectics. All energies per mole of oxide formula unit.

This is the honest phase module: it makes no assumption about melting type. Absolute
temperatures remain limited by the solid model (polymorphs, Neumann-Kopp Cp) - the point
here is correct invariant CLASSIFICATION.
"""
import sys
...
def load(dat):
    ...
    return db, db.phase_index("CAO-SIO2-LIQUID")
...
def hull_bracket(hull, x):
    """Return the two hull vertices bracketing composition x (their labels)."""
...
def compound_is_stable(db, p, name, T):
    ...
    Returns (stable, coexisting_label_pair_at_xC)."""
    ...
    # interpolate the competing hull value at xC
...
def classify_compound(db, p, name, lo=1400.0, hi=2700.0):
    xC = pdg.SOLIDS[name][1] / (pdg.SOLIDS[name][0] + pdg.SOLIDS[name][1])
    # highest T at which the compound is on the hull
    ...
    # what does it coexist with just above Tmax? classify melting type
    left, right = coex
...
if __name__ == "__main__":
    dat = Path(sys.argv[1]) if len(sys.argv) > 1 else HERE / "CaO-SiO2-liquid.dat"
    db, p = load(dat)
    print(f"Hull invariant classification from {dat.name}")
    print("=" * 70)
    print("Compound melting (type is the physics; T is solid-model-limited):")
    known = {"CS(pseudowoll)": "congruent 1817 K",
             "C2S(gamma)": "congruent 2403 K",
             "C3S2(rankinite)": "PERITECTIC ~1737 K",
             "C3S": "peritectic / limited stability window"}
    for name in ("CS(pseudowoll)", "C2S(gamma)", "C3S2(rankinite)", "C3S"):
    ...
    for n1, n2 in (("CS(pseudowoll)", "C3S2(rankinite)"),
                   ("CS(pseudowoll)", "SiO2(cristobalite)")):
