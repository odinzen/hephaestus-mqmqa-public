"""Component activities of the CaO-SiO2 liquid from the MQMQA engine.
...
G_tot(n_CaO, n_SiO2) = GM(x_SiO2) * n_atoms with respect to n_i at fixed n_j. The
...
amplifies it by 1e4). For the CaO-SiO2 binary the quadruplet distribution has a
...
R = 8.3145
ATOMS = {"CaO": 2.0, "SiO2": 3.0}  # atoms per oxide formula unit
...
def _g_total(inp, n_cao, n_sio2):
    n_atoms = ATOMS["CaO"] * n_cao + ATOMS["SiO2"] * n_sio2
    return gm(inp, n_sio2 / (n_cao + n_sio2)) * n_atoms
...
def g_pure_liquid(inp, which):
    ...
    return gm(inp, 1e-9) * ATOMS["CaO"]


def activities(inp, x_sio2, T, h=2e-4):
    """(a_CaO, a_SiO2) at oxide fraction x_sio2 and temperature T, liquid ref."""
    n_cao, n_sio2 = 1.0 - x_sio2, x_sio2
    mu_sio2 = (_g_total(inp, n_cao, n_sio2 + h)
               - _g_total(inp, n_cao, n_sio2 - h)) / (2.0 * h)
    mu_cao = (_g_total(inp, n_cao + h, n_sio2)
              - _g_total(inp, n_cao - h, n_sio2)) / (2.0 * h)
    a_cao = np.exp((mu_cao - g_pure_liquid(inp, "CaO")) / (R * T))
    a_sio2 = np.exp((mu_sio2 - g_pure_liquid(inp, "SiO2")) / (R * T))
    return float(a_cao), float(a_sio2)


def delta_g_mix(inp, x_sio2):
    """Integral Gibbs energy of mixing per mole of oxide formula (liquid ref).

    G_mix = G(x) - (1-x) G_CaO_pure - x G_SiO2_pure, all per oxide formula unit.
    """
    n_atoms = ATOMS["CaO"] * (1 - x_sio2) + ATOMS["SiO2"] * x_sio2
    g = gm(inp, x_sio2) * n_atoms  # per (1-x) CaO + x SiO2, i.e. per oxide formula
    g_cao = g_pure_liquid(inp, "CaO")
