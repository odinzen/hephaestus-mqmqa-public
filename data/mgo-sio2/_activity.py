"""Component activities of the MgO-SiO2 liquid from the MQMQA engine.
...
amplifies it by 1e4). For the MgO-SiO2 binary the quadruplet distribution has a
...
R = 8.3145
ATOMS = {"MgO": 2.0, "SiO2": 3.0}  # atoms per oxide formula unit
...
def _g_total(inp, n_cao, n_sio2):
    n_atoms = ATOMS["MgO"] * n_cao + ATOMS["SiO2"] * n_sio2
...
def activities(inp, x_sio2, T, h=2e-4):
    ...
    a_cao = np.exp((mu_cao - g_pure_liquid(inp, "MgO")) / (R * T))
...
def delta_g_mix(inp, x_sio2):
    ...
    """
    n_atoms = ATOMS["MgO"] * (1 - x_sio2) + ATOMS["SiO2"] * x_sio2
    g = gm(inp, x_sio2) * n_atoms  # per (1-x) MgO + x SiO2, i.e. per oxide formula
    g_cao = g_pure_liquid(inp, "MgO")
