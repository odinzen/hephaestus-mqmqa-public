...
def main():
    ...
    ax.fill_between(XS, lo, hi, color="0.78", lw=0)
    ax.plot(XS, np.nanpercentile(liq, 50, 0), "k-", lw=1.6, label="liquidus")
    ...
    ax.fill_betweenx(TSOL, slo, shi, color="0.78", lw=0, label="90% posterior band")
    ax.plot(np.nanpercentile(sol, 50, 0), TSOL, "k--", lw=1.4, label="fcc solvus")
    ax.plot([0.887], [654], "o", mfc="white", mec="black", ms=6, label="eutectic (Massalski)")
    ax.text(0.09, 893, "LIQUID", fontsize=9, color="0.35")
    ax.text(0.05, 603, "FCC", fontsize=9, color="0.35")
    ax.text(0.85, 573, "FCC\n+ HCP", fontsize=9, color="0.35", ha="center")
    ax.set_xlabel("x(Zn)"); ax.set_ylabel("Temperature (K)")
    ax.set_xlim(0, 1); ax.set_ylim(390, 940)
